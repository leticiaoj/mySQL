...
SELECT * FROM tb_produtos INNER JOIN tb_categorias ON tb_produtos.categoria_id = 3;

SELECT * FROM tb_produtos INNER JOIN tb_usuarios ON tb_produtos.usuario_id ORDER BY tb_produtos.nome;
...
WHERE tb_produto.id = 1;
-- quem manda no inner join é a tabela que tem a chave estrangeira, ela que inverte a ordem

DELETE FROM tb_categorias WHERE id = 5;